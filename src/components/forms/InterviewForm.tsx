...
export default function InterviewForm({ initial, applications, onSubmit, onCancel }: Props) {
  ...
  return (
    <form
      onSubmit={form.handleSubmit((values) => onSubmit({ ...values, scheduledAt: scheduledValue || values.scheduledAt }))}
      className="space-y-3"
    >
      ...
      <div className="grid grid-cols-2 gap-3"><div><label className="mb-1 block text-sm">轮次</label><Input {...form.register('round')} /></div><div><label className="mb-1 block text-sm">时间</label><Input type="time" value={timeOnly} onChange={(e) => setTimeOnly(e.target.value)} /></div></div>
      <div>
        <label className="mb-1 block text-sm">日期</label>
        {/* v0.2.1: Interview 日期改为 DatePicker */}
        <DatePicker value={dateOnly || undefined} onChange={(value) => setDateOnly(value ?? '')} />
      </div>
