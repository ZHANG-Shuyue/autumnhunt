...
export default function ApplicationForm({ id = 'application-form', initial, companies, onSubmit, fixedCompanyId }: Props) {
  ...
  return (
    <form id={id} onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div><label className="mb-1 block text-sm">简历版本</label><Input {...form.register('resumeFile')} /></div><div><label className="mb-1 block text-sm">准备文档</label><Input {...form.register('preparationDocUrl')} /></div></div>
